...
<article class="w-[330px] h-[{{$height}}] flex flex-col items-center bg-[#343444] rounded-[20px] p-[20px]">
    <div class="bg-[#7A798A] relative w-[290px] h-[290px] rounded-[20px] overflow-hidden">
        <form action=""
              method="POST">
            @csrf
            <input type="hidden" value="{{\App\Models\Item::class}}" name="likeable_type">
            <input type="hidden" @if(isset($item)) value="{{$item->id}}" @endif name="likeable_id">
            <button {{$attributes->merge(['id' => 'like-'.$item->id])}} class="absolute w-[64px] h-[28px] bg-[#14141F] rounded-[10px] top-[14px] right-[12px] flex justify-center items-center gap-x-[5px] text-white hover:border-white hover:border-[1.5px] transition-[border] ease-linear cursor-pointer">
...
@if(isset($item))
    <script>
        document.querySelector('#like-{{$item->id}}').addEventListener('click', (e) => {
            e.preventDefault();

            fetch('{{ action([\App\Http\Controllers\LikeController::class, 'store'], ['item' => $item]) }}'
                , {method: 'POST',
                    headers: {
                ...
                .then(data => {
                    document.querySelector('#like-{{$item->id}} .like_count').innerHTML = data.count;
                })
        })
    </script>
@endif
