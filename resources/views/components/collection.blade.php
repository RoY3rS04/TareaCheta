...
<article class="flex flex-col gap-y-[15px] p-5 bg-[#343444] w-[450px] rounded-[10px]">
    <div class="grid grid-cols-6 gap-[10px]">
        @foreach($collectionImages as $index => $img)
            <div class="bg-[#7A798A] @if($index < 2) col-span-3 @else col-span-2 @endif h-[131px] rounded-[10px] overflow-hidden"><img src="{{$img->getFirstMediaUrl('items_images')}}" alt="" class="w-full h-full object-cover object-center"></div>
        @endforeach
    </div>
    <div class="flex items-center justify-between">
        <div class="flex gap-x-2.5 items-center">
            <x-user :user_image="$author->getFirstMediaUrl('user_images')"/>
        ...
        <form action=""
            ...
            <input type="hidden" @if(isset($collection)) value="{{$collection->id}}" @endif name="likeable_id">
            <button class="w-[64px] h-[28px] bg-[#14141F] rounded-[10px] top-[14px] right-[12px] flex justify-center items-center gap-x-[5px] text-white hover:border-white hover:border-[1.5px] transition-[border] ease-linear cursor-pointer">
